...
const FormWidget = (props: FormWidgetProps): JSX.Element|null => {
  ...
  const getPreviousAndNextStepIds = (currentStepId: string): {
    previousStepId: string|null,
    nextStepId: string|null,
  } => {
    const stepIds = steps.map((step) => step.id);
    const indexOfCurrentStepId = stepIds.indexOf(currentStepId);
    const previousStepId = indexOfCurrentStepId === -1 ? null : (indexOfCurrentStepId === 0 ? null : stepIds[indexOfCurrentStepId - 1]);
    const nextStepId = indexOfCurrentStepId === -1 ? null : (indexOfCurrentStepId === stepIds.length - 1 ? null : stepIds[indexOfCurrentStepId + 1]);
    return {
      previousStepId,
      nextStepId,
    };
